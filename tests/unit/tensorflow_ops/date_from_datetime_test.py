...
"""Tests for BigQuery DateFromDatetime custom op."""

from bigquery_ml_utils.tensorflow_ops import date_ops
import tensorflow as tf


class DateFromDatetimeTest(tf.test.TestCase):
  ...
  def test_date_from_datetime(self):
    ...
    self.assertAllEqual(
        date_ops.date_from_datetime(datetime),
        tf.constant(['2023-02-02', '2008-12-25']),
    )

  def test_date_from_datetime_invalid_datetime(self):
    datetime = tf.constant(
    ...
    with self.assertRaisesRegex(
      ...
      self.evaluate(date_ops.date_from_datetime(datetime))
