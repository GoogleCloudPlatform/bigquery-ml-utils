...
#ifndef THIRD_PARTY_PY_BIGQUERY_ML_UTILS_SQL_UTILS_BASE_MAP_UTIL_H_
...
// variants will crash the process with a SQL_CHECK() failure on error,
// including the offending key/data in the log message. The NoPrint variants
// will not include the key/data in the log output under the assumption that
// it's not a printable type.
//
...
#include "sql_utils/base/map_traits.h"
#include "sql_utils/base/no_destructor.h"
...
const V& ValueInitializedDefault() {
  static const bigquery_ml_utils_base::NoDestructor<V>
      value_initialized_default{};
...
const MapUtilMappedT<M>& FindOrDie(const M& m, const KeyType& key) {
  auto it = m.find(key);
  SQL_CHECK(it != m.end()) << "Map key not found: " << key;
  return bigquery_ml_utils_base::subtle::GetMapped(*it);
}
...
const MapUtilMappedT<M>& FindOrDieNoPrint(const M& m, const KeyType& key) {
  auto it = m.find(key);
  SQL_CHECK(it != m.end()) << "Map key not found";
  return bigquery_ml_utils_base::subtle::GetMapped(*it);
}
...
const MapUtilMappedT<M>& FindWithDefault(const M& m, const KeyType& key) {
  auto it = m.find(key);
  if (it != m.end()) return bigquery_ml_utils_base::subtle::GetMapped(*it);
...
bool FindCopy(const M& m, const Key& key, Value* value) {
  ...
  if (value) *value = bigquery_ml_utils_base::subtle::GetMapped(*it);
  return true;
}
...
bool ContainsKeyValuePair(const M& m, const Key& key, const Value& value) {
  ...
  for (auto it = range.first; it != range.second; ++it) {
    if (bigquery_ml_utils_base::subtle::GetMapped(*it) == value) {
      return true;
    }
  }
  return false;
...
void InsertOrDie(M* m, const MapUtilInitT<M>& value) {
  SQL_CHECK(InsertIfNotPresent(m, value)) << "duplicate value: " << value;
}

// Same as above except doesn't log the value on error.
template <typename M>
void InsertOrDieNoPrint(M* m, const MapUtilInitT<M>& value) {
  SQL_CHECK(InsertIfNotPresent(m, value)) << "duplicate value.";
}
...
void InsertOrDie(M* m, const MapUtilKeyT<M>& key,
  ...
  SQL_CHECK(InsertIfNotPresent(m, key, data)) << "duplicate key: " << key;
}
...
void InsertOrDieNoPrint(M* m, const MapUtilKeyT<M>& key,
                        const MapUtilMappedT<M>& data) {
  SQL_CHECK(InsertIfNotPresent(m, key, data)) << "duplicate key.";
}
...
auto InsertKeyOrDie(M* m, const MapUtilKeyT<M>& key) ->
  ...
  SQL_CHECK(res.second) << "duplicate key: " << key;
  return bigquery_ml_utils_base::subtle::GetMapped(*res.first);
}
...
MapUtilMappedT<M> EraseKeyReturnValuePtr(M* m, const MapUtilKeyT<M>& key) {
  ...
  if (it == m->end()) return nullptr;
  MapUtilMappedT<M> v =
      std::move(bigquery_ml_utils_base::subtle::GetMapped(*it));
  m->erase(it);
  return v;
}
...
void InsertKeysFromMap(const M& m, KeyContainer* key_container) {
  SQL_CHECK(key_container != nullptr);
  for (const auto& kv : m) {
...
void AppendValuesFromMap(const M& m, ValueContainer* value_container) {
  SQL_CHECK(value_container != nullptr);
  for (const auto& kv : m) {
...
#endif  // THIRD_PARTY_PY_BIGQUERY_ML_UTILS_SQL_UTILS_BASE_MAP_UTIL_H_