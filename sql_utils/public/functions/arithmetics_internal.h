...
#ifndef THIRD_PARTY_PY_BIGQUERY_ML_UTILS_SQL_UTILS_PUBLIC_FUNCTIONS_ARITHMETICS_INTERNAL_H_
#define THIRD_PARTY_PY_BIGQUERY_ML_UTILS_SQL_UTILS_PUBLIC_FUNCTIONS_ARITHMETICS_INTERNAL_H_
...
inline Saturated<T>& Saturated<T>::Mul(T t2) {
  ...
  if (t_ < 0 && t2 > 0) {
    ...
    T r = Min() % t2;
    if (r > 0) {
      SQL_CHECK_LT(r, t2);  // or % is broken
      SQL_CHECK_LT(q, 0);
...
inline Saturated<T>& Saturated<T>::Rem(T t2) {
  ...
  if (!t3.IsValid()) {
    ...
    T t_new = t_ % t2;
    SQL_CHECK_EQ(t3.Value() * t2 + t_new, t_);
    t_ = t_new;
  }
  return *this;
}
...
#endif  // THIRD_PARTY_PY_BIGQUERY_ML_UTILS_SQL_UTILS_PUBLIC_FUNCTIONS_ARITHMETICS_INTERNAL_H_
