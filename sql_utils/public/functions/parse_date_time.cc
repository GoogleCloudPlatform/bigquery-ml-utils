...
absl::Status CanonicalizeISODateParseContext(
  ...
  SQL_RET_CHECK(!date_parse_context->non_iso_date_part_present);
  SQL_RET_CHECK_GE(iso_year_idx, 0);
  ...
  if (iso_week_idx >= 0) {
    ...
    SQL_RET_CHECK_LT(iso_dayofyear_idx, 0);
    canonicalized_date_parse_context.elements.push_back(
  ...
  if (iso_dayofyear_idx >= 0) {
    // ISO week is mutually exclusive with day of year.
    SQL_RET_CHECK_LT(iso_week_idx, 0);
    canonicalized_date_parse_context.elements.push_back(
...
absl::Status CanonicalizeNonISODateParseContext(
  ...
  if (week_idx >= 0) {
    ...
    SQL_RET_CHECK_LT(dayofyear_idx, 0);
    canonicalized_date_parse_context.elements.push_back(
...
absl::Status ParseWeekdayFromElement(const ParseElementInfo& weekday_element,
                                     int* weekday) {
  SQL_RET_CHECK(weekday_element.fmt == 'A' || weekday_element.fmt == 'a' ||
...
static absl::Status ParseWeek(const ParseElementInfo& week_element, int* week,
  ...
  if (week_element.fmt == 'U') {
    ...
    *week_start_day = absl::Weekday::monday;
    ...
    SQL_RET_CHECK_FAIL() << "Unexpected week parse element %" << week_element.fmt;
  }
  return absl::OkStatus();
}
...
static absl::Status ParseISOYear(const ParseElementInfo& year_element,
  ...
  if (year_element.fmt == 'g') {
    ...
    SQL_RET_CHECK_FAIL() << "unexpected format_element: " << year_element.fmt;
  }
  return absl::OkStatus();
}
...
static absl::StatusOr<int64_t> FirstWeekNumberOfYear(int64_t year,
  ...
  SQL_RET_CHECK_FAIL() << "Unexpected format element: " << element;
}
...
static absl::Status CheckWeekNumberValidityForYear(int64_t year, int week_number,
                                                   const char element) {
  SQL_ASSIGN_OR_RETURN(int64_t first_week_number,
                   FirstWeekNumberOfYear(year, element));
  SQL_ASSIGN_OR_RETURN(int64_t last_week_number, LastWeekNumberOfYear(year, element));
...
static absl::Status ComputeDateFromISOYearAndDayOfYear(
  ...
  int iso_year = -1;
  SQL_RETURN_IF_ERROR(ParseISOYear(iso_year_element, &iso_year));
  ...
  *civil_day =
  ...
  int dayofyear;
  SQL_RETURN_IF_ERROR(ParseDayOfYear(dayofyear_element.value(),
...
static absl::Status ComputeDateFromISOYearWeekAndWeekday(
  ...
  int iso_week;
  SQL_RET_CHECK_EQ(week_element.fmt, 'V');
  ...
  if (weekday_element.has_value()) {
    SQL_RETURN_IF_ERROR(ParseWeekdayFromElement(weekday_element.value(), &weekday));
  }
...
static absl::Status ComputeDateFromYearWeekAndWeekday(
  ...
  SQL_RETURN_IF_ERROR(ParseWeek(week_element, &week, &week_start_day));
  SQL_RET_CHECK_GE(week, 0);
  SQL_RET_CHECK_LE(week, 53);

  SQL_RETURN_IF_ERROR(CheckWeekNumberValidityForYear(year, week, week_element.fmt));
...
static absl::Status UpdateYearMonthDayIfNeeded(
  ...
  // during processing.
  SQL_RETURN_IF_ERROR(CanonicalizeDateParseContext(date_parse_context));
...
static absl::Status ParseTime(absl::string_view format,
  ...
  absl::Time base_time;
  SQL_RETURN_IF_ERROR(ParseTime(format, timestamp_string, default_timezone,
...
absl::Status ParseStringToTimestamp(absl::string_view format_string,
                                    ...
                                    bool parse_version2,
                                    absl::Time* timestamp) {
  SQL_RETURN_IF_ERROR(ParseTime(format_string, timestamp_string, default_timezone,
...
absl::Status ParseStringToTime(absl::string_view format_string,
                               ...
                               TimeValue* time) {
  SQL_CHECK(scale == kNanoseconds || scale == kMicroseconds);
  SQL_RETURN_IF_ERROR(ValidateTimeFormat(format_string));

  absl::Time base_time;
  SQL_RETURN_IF_ERROR(ParseTime(format_string, time_string, absl::UTCTimeZone(),
...
absl::Status ParseStringToDatetime(absl::string_view format_string,
  ...
  SQL_CHECK(scale == kNanoseconds || scale == kMicroseconds);
  SQL_RETURN_IF_ERROR(ValidateDatetimeFormat(format_string));

  absl::Time base_time;
  SQL_RETURN_IF_ERROR(ParseTime(format_string, datetime_string, absl::UTCTimeZone(),
