...
#include "sql_utils/public/time_zone_util.h"
...
bool IsValidDate(int32_t date) {
  return date >= bigquery_ml_utils::types::kDateMin &&
         date <= bigquery_ml_utils::types::kDateMax;
}
...
static bool ParsePrefixToTimeParts(absl::string_view str, TimestampScale scale,
  ...
  if (str[*idx] == '.') {
    ...
    SQL_CHECK_LE(num_parsed_subsecond_digits, 9);
...
static absl::Status AddTimestampNanos(int64_t nanos, absl::TimeZone timezone,
  ...
  if (part == NANOSECOND) {
    ...
    int64_t micros_out;
    SQL_RETURN_IF_ERROR(AddTimestampInternal(micros, kMicroseconds, timezone, part,
                                         interval, &micros_out));
...
static absl::Status FormatTimestampToStringInternal(
  ...
  // FormatTime() because SQL behavior is different than FormatTime()
  // behavior.
  SQL_RETURN_IF_ERROR(internal_functions::ExpandPercentZQJ(
...
absl::Status LastDayOfDatetime(const DatetimeValue& datetime,
  ...
  SQL_RETURN_IF_ERROR(LastDayOfDate(date, part, output));
  return absl::OkStatus();
}
...
bool IsValidTimestamp(int64_t timestamp, TimestampScale scale) {
  switch (scale) {
    case kSeconds:
      return timestamp >= bigquery_ml_utils::types::kTimestampMin / 1000000 &&
             timestamp <= bigquery_ml_utils::types::kTimestampMax / 1000000;
    case kMilliseconds:
      return timestamp >= bigquery_ml_utils::types::kTimestampMin / 1000 &&
             timestamp <= bigquery_ml_utils::types::kTimestampMax / 1000;
    case kMicroseconds:
      return timestamp >= bigquery_ml_utils::types::kTimestampMin &&
             timestamp <= bigquery_ml_utils::types::kTimestampMax;
    case kNanoseconds:
...
bool IsValidTime(absl::Time time) {
  static const absl::Time kTimeMin =
      absl::FromUnixMicros(bigquery_ml_utils::types::kTimestampMin);
  static const absl::Time kTimeMax =
      absl::FromUnixMicros(bigquery_ml_utils::types::kTimestampMax + 1);
...
absl::Status FormatDateToString(
  ...
  SQL_RETURN_IF_ERROR(FormatTimestampToString(date_format_string, date_timestamp,
...
absl::Status FormatDatetimeToStringWithOptions(
  ...
  SQL_RETURN_IF_ERROR(FormatTimestampToString(datetime_format_string,
...
absl::Status FormatTimeToString(absl::string_view format_string,
  ...
  // relevant.
  SQL_RETURN_IF_ERROR(FormatTimestampToString(
...
absl::Status ConvertStringToDate(absl::string_view str, int32_t* date) {
  ...
  *date = CivilDayToEpochDays(civil_day);
  SQL_DCHECK(IsValidDate(*date));  // Invariant if MakeDate() succeeds.
  return absl::OkStatus();
}
...
absl::Status ConvertStringToTimestamp(absl::string_view str,
                                      ...
                                      int64_t* timestamp) {
  absl::Time base_time;
  SQL_RETURN_IF_ERROR(ConvertStringToTimestamp(str, default_timezone, scale,
...
absl::Status ConvertStringToTimestamp(absl::string_view str,
  ...
  absl::TimeZone timezone;
  SQL_RETURN_IF_ERROR(ParseStringToTimestampParts(
...
absl::Status ConvertTimestampToTime(absl::Time base_time,
  ...
  SQL_RET_CHECK(scale == kNanoseconds || scale == kMicroseconds);
  if (IsValidTime(base_time)) {
    const absl::TimeZone::CivilInfo info = timezone.At(base_time);
    if (scale == kNanoseconds) {
...
absl::Status ConvertDateToTimestamp(int32_t date, TimestampScale scale,
  ...
  absl::Time base_time;
  SQL_RETURN_IF_ERROR(ConvertDateToTimestamp(date, timezone, &base_time));
...
absl::Status ConvertProto3TimestampToTimestamp(
  ...
  absl::Time time;
  SQL_RETURN_IF_ERROR(ConvertProto3TimestampToTimestamp(input_timestamp, &time));
...
absl::Status ConvertProto3TimestampToTimestamp(
  ...
  auto result_or = bigquery_ml_utils_base::DecodeGoogleApiProto(input_timestamp);
  if (!result_or.ok()) {
    return MakeEvalError() << "Invalid Proto3 Timestamp input: "
                        << input_timestamp.DebugString();
  }
  *output = result_or.value();
  // DecodeGoogleApiProto enforces the same valid timestamp range as SQL.
  // This check is meant to give us protection in case the contract of
  // DecodeGoogleApiProto changes in the future.
  SQL_DCHECK(IsValidTime(*output));
  return absl::OkStatus();
}
...
absl::Status ConvertTimestampToProto3Timestamp(
  ...
  // EncodeGoogleApiProto enforces the same valid timestamp range as SQL.
  return bigquery_ml_utils_base::EncodeGoogleApiProto(input_timestamp, output);
}
...
absl::Status AddDate(int32_t date, DateTimestampPart part, int64_t interval,
                     int32_t* output) {
  // The interval is an int64_t to match the SQL function signature.
  ...
  SQL_RETURN_IF_ERROR(AddDateOverflow(date, part, static_cast<int32_t>(interval),
                                  output, &had_overflow));
  if (had_overflow) {
    return MakeAddDateOverflowError(date, part, interval);
  }
...
absl::Status AddDate(int32_t date, bigquery_ml_utils::IntervalValue interval,
  ...
  SQL_RETURN_IF_ERROR(ConstructDatetime(date, TimeValue(), &datetime));
  SQL_RETURN_IF_ERROR(AddDatetime(datetime, interval, output));
  return absl::OkStatus();
}
...
absl::StatusOr<IntervalValue> IntervalDiffDatetimes(
  ...
  int64_t seconds;
  SQL_RETURN_IF_ERROR(DiffDatetimes(datetime1, datetime2, SECOND, &seconds));
...
static absl::Status AddDatetimeInternal(
  ...
  if (CheckValidAddTimestampPart(part, false /* is_legacy */).ok()) {
    ...
    SQL_RETURN_IF_ERROR(ConvertTimestampToDatetime(result_timestamp,
...
absl::Status SubDatetime(const DatetimeValue& datetime, DateTimestampPart part,
  ...
  if (interval == std::numeric_limits<int64_t>::lowest()) {
    SQL_RETURN_IF_ERROR(AddDatetimeInternal(datetime, part,
                                        std::numeric_limits<int64_t>::max(),
                                        output, error_maker));
...
absl::Status AddDatetime(DatetimeValue datetime,
  ...
  if (interval.get_days() != 0) {
    SQL_RETURN_IF_ERROR(AddDatetime(datetime, DAY, interval.get_days(), &datetime));
  }
  bool had_overflow = false;
  if (interval.get_micros() != 0) {
    ...
    // result.
    SQL_RETURN_IF_ERROR(AddDatetimeInternal(datetime, MICROSECOND,
    ...
    if (had_overflow) {
      SQL_RETURN_IF_ERROR(AddDatetime(datetime, MICROSECOND,
                                  interval.get_micros() + 1, &datetime));
    }
  }
  if (interval.get_nano_fractions() != 0) {
    SQL_RETURN_IF_ERROR(AddDatetime(datetime, NANOSECOND,
                                interval.get_nano_fractions(), &datetime));
  }
  if (had_overflow) {
    SQL_RETURN_IF_ERROR(AddDatetime(datetime, MICROSECOND, -1, &datetime));
  }
...
absl::Status AddTimestamp(absl::Time timestamp, absl::TimeZone timezone,
                          ...
                          absl::Time* output) {
  bool had_overflow_unused;
  SQL_RETURN_IF_ERROR(AddTimestampInternal(timestamp, timezone, part, interval,
...
absl::Status AddTimestamp(absl::Time timestamp, absl::TimeZone timezone,
  ...
  if (interval.get_days() != 0) {
    SQL_RETURN_IF_ERROR(AddTimestamp(timestamp, timezone, DAY, interval.get_days(),
                                 &timestamp));
  }
  bool had_overflow = false;
  if (interval.get_micros() != 0) {
  ...
  if (interval.get_nano_fractions() != 0) {
    SQL_RETURN_IF_ERROR(AddTimestamp(timestamp, timezone, NANOSECOND,
...
absl::Status SubTimestamp(int64_t timestamp, TimestampScale scale,
  ...
  SQL_RETURN_IF_ERROR(AddTimestampInternal(timestamp, scale, timezone, part,
                                       -interval, output));
  if (!IsValidTimestamp(*output, scale)) {
    return MakeSubTimestampOverflowError(timestamp, part, interval, scale,
                                         timezone);
...
static void AddOnField(int64_t interval, int64_t radix, int* field,
                       int64_t* carry) {
  // Expected invariants.
  SQL_DCHECK_LE(0, *field);
  SQL_DCHECK_LT(*field, radix);
  SQL_DCHECK_LE(radix, 1000000000);  // number of nanos in a second.
...
absl::Status SubTime(const TimeValue& time, DateTimestampPart part,
                     int64_t interval, TimeValue* output) {
  if (interval == std::numeric_limits<int64_t>::lowest()) {
    SQL_RETURN_IF_ERROR(AddTimeInternal(
...
absl::StatusOr<IntervalValue> IntervalDiffTimes(const TimeValue& time1,
  ...
  int64_t nanos;
  SQL_RETURN_IF_ERROR(DiffTimes(time1, time2, NANOSECOND, &nanos));
...
absl::StatusOr<IntervalValue> IntervalDiffTimestamps(absl::Time timestamp1,
  ...
  SQL_RET_CHECK(rem == absl::ZeroDuration());
...
// Expand "%Z" in <format_string> to the SQL-defined format:
//   'UTC[+/-HHMM]'
// The format produced by absl::FormatTime() is different from the SQL
...
// SQL's extension to strftime format strings.  We have
...
// SQL extension to strftime, and we have to do it here because
...
//
// SQL does not support rendering numeric timezones with sub-minute
